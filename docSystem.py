...
class Database:
    ...
    def add_item(self, data):
        try:
            self.cursor.execute("""
                INSERT INTO impressos (id, nome, categoria, origem, descricao, status, image_path, created_at)
                VALUES (:id, :nome, :categoria, :origem, :descricao, :status, :image_path, :created_at)
            """, data)
            self.conn.commit()
            return True
...
class WebDocsGenerator:
    def generate(self, data_list, output_folder="WebDocs_Sistema"):
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
        
        # Copiar imagens para pasta web
...
class App(ctk.CTk):
    ...
    def _setup_ui(self):
        ...
        self.combo_status = ctk.CTkComboBox(self.left_frame, values=["Ativo", "Migrar para BI", "Obsoleto", "Redundante", "Revisar"])
